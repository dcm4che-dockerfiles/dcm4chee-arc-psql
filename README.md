...
- a not secured version (Tag Name: `5.8.1`),
- a version with secured UI and secured RESTful services (Tag Name: `5.8.1-secure`),
- a version with secured UI, but not secured RESTful services (Tag Name: `5.8.1-secure-ui`),
- a not secured version with pre-configured [GELF Logger](http://logging.paluch.biz/examples/wildfly.html)
  to emit System logs to Logstash (Tag Name: `5.8.1-logstash`),
- a version with pre-configured [GELF Logger](http://logging.paluch.biz/examples/wildfly.html) and with secured UI
  and secured RESTful services (Tag Name: `5.8.1-logstash-secure`) and
- a version with pre-configured [GELF Logger](http://logging.paluch.biz/examples/wildfly.html) and with secured UI,
  but not secured RESTful services (Tag Name: `5.8.1-logstash-secure-ui`).
...
> $docker run --name dcm4chee-arc \
           ...
           -d dcm4che/dcm4chee-arc-psql:5.8.1-logstash-secure-ui
```
...
services:
  ...
  logstash:
    ...
    volumes:
      ...
      - /etc/localtime:/etc/localtime
  dcm4chee-arc:
    image: dcm4che/dcm4chee-arc-psql:5.8.1-logstash-secure-ui
    ports:
      - "8080:8080"
      - "9990:9990"
      - "11112:11112"
